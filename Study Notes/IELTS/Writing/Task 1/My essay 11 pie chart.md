You should spend about 20 minutes on this task.

**_The two pie charts below show the marital status of the populations of Canada and Australia._**

**_Summarise the information by selecting and reporting the main features, and make comparisons where relevant._**

You should write at least 150 words.

![[Pasted image 20260109110931.png]]

canada:
- more than half are separated
- a quarter are married
- a quarter is never married, widowed, divorced, married to the same sex 

Australia
- almost half are maried
- less than half are separated
- the ramaining are single, widowed, divorced
- no same sex marriage

Marital status in Canada and Australia is reported by the two given pie charts.

Overall, it is apparent that separation takes more than half in the statistics of marital status in Canada and more than a quarter of Canadians are married. On the other hand, separation is significantly less ~~==prevelant==~~ prevalent in Autralia than it is on Canada, and almost half of the citizens are married.

Separation without divorce is the most common status in Canada with a figure of 53% and 29% are married. The remaining 18% is shared by singles, widows, divorced and same sex marriages with 7%, 7%, 1% and 3% respectively.

Australia on the other hand have 41% separated individuals and 46% are married, a slight difference of 5% between these two figures comparison with the other two figures of Canada. The remaining figures in Autralia are close to the ones of the latter country, 5% are single, 6% are widowed, 2% are divorced; conversly, there are no reported same sex marriages.
